...
#2 
  ...
  PPLakesData <- PPLakesData %>% mutate(sampledate = as.Date(sampledate, format = "%Y-%m-%d"))
  NRLitterData <- NRLitterData %>% mutate(collectDate = as.Date(collectDate, format = "%Y-%m-%d"))
...
Lakes_P_plot <- ggplot(data = PPLakesData, aes(x = tp_ug, y = po4, color = lakename)) +
                       ...
                       ylim(0, 50) + 
                       labs(x = "Total Phosphorus (ug/L)", y = "Phosphate (ug/L)", color = "Lake Name") +
...
```{r}
#5
Lakes_temp_plot <- ggplot(data = PPLakesData, aes(x = as.factor(month), y = temperature_C, color = lakename)) +
...
Lakes_TP_plot <- ggplot(data = PPLakesData, aes(x = as.factor(month), y = tp_ug, color = lakename)) +
                          geom_boxplot(alpha = 0.7) +
                          labs(x = "", y = "TP (ug/L)", color = "Lake Name") + #keep the yaxis name short so it fits
                          scale_color_manual(values = wes_palettes$Cavalcanti1) + 
                          theme(legend.position = "none")

Lakes_TN_plot <- ggplot(data = PPLakesData, aes(x = as.factor(month), y = tn_ug, color = lakename)) +
                          geom_boxplot(alpha = 0.7) +
                          labs(x = "Month", y = "TN (ug/L)", color = "Lake Name") + #keep the yaxis name short 
                          scale_color_manual(values = wes_palettes$Cavalcanti1)

plot_grid(Lakes_temp_plot, Lakes_TP_plot, Lakes_TN_plot, nrow = 3, align = 'v', rel_heights = c(1, 1, 1.45))
