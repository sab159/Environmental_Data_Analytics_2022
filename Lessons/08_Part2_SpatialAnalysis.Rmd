...
mapview::mapview(huc8_sf, zcol = "ACRES")
...
#Create a map
mapview(huc8_sf, color = "purple", col.regions = "purple") + 
      mapview(UpperNeuse_sf, color = "orange", col.regions = "orange", alpha = 1, alpha.regions = 1) 

ggplot() + 
  geom_sf(data = huc8_sf, fill = "orange") +
  geom_sf(data = UpperNeuse_sf, fill = "purple")
...
HUC12_EnviroAtlas.data <- read.csv("https://raw.githubusercontent.com/ENV859/EnviroAtlasData/main/Wind_Energy.csv",
                                   colClasses = c("HUC_12" = "factor"))

head(HUC12_EnviroAtlas.sf)

EnviroAtlas.data <- HUC12_EnviroAtlas.data %>% 
                    mutate(HUC_8 = substr(HUC_12, 1, 8)) %>% 
                    group_by(HUC_8) %>% 
                    summarise(TotAvgWindEnergy = sum(AvgWindEnergy),
                              TotkWhkm2day = sum(kWhkm2day))


#Join to HUC_8 features
HUC8_WindEn.sf <- merge(EnviroAtlas.data, huc8_sf, by = "HUC_8") %>% 
                  st_as_sf(crs = st_crs(huc8_sf)) #Why did sf-ness get dropped? 

#View the outputs
mapview::mapview(HUC8_WindEn.sf, zcol = "TotAvgWindEnergy")

ggplot(HUC8_WindEn.sf, aes(fill = TotAvgWindEnergy)) + 
  geom_sf()
