...
PP_surf <- PeterPaul.chem.nutrients %>% filter(depth == 0)

ggplot(PP_surf, aes(x = sampledate, y = temperature_C, color = year4)) +
       geom_point() +
...
ggplot(PeterPaul.chem.nutrients, aes(x = sampledate, y = temperature_C, color = depth)) +
      geom_point(size = 0.5) +
      scale_color_viridis(option = "magma", direction = -1) + #negative one makes deeper darker, which makes sense. 
