...
```{r}
#Exercise 2:
ggplot(data = EPAair, aes(x = Ozone, color = as.factor(Year))) +
  geom_density(aes(fill = as.factor(Year)), alpha = 0.4) +
  ...
  scale_color_manual(values = wes_palettes$Royal1[2:1]) + 
  scale_fill_manual(values = wes_palettes$Royal1[2:1])

ggplot(data = EPAair, aes(x = as.factor(Year), y = Ozone)) +
  geom_boxplot() +
  labs(x = "Year", y = "Ozone AQI Value", title = "Ozone Spread by Year")
