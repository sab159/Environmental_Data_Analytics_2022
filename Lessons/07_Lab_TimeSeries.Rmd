...
#Exercise 5: Check if there is any missing data for both temperature and rainfall series.
  ...
  # Create a sequence of indexed dates
  fullDates <- seq.Date(min(BRA_complete$Date), max(BRA_complete$Date), by = "month")

  # join sequence with dataset and check for missing dataset
  date_check <- merge(BRA_complete, fullDates, by.x = "Date", all.y = TRUE)
  date_check <- left_join(fullDates, BRA_complete) #FIGURE THIS OUT
  
  #check NAs because matched two datasets 
...
# Hard to discern any trend - useful to have stats as well
...
rain_timeseries <- ts(BRA_complete$Rainfall, start = min(BRA_complete$Date), frequency = 12)
temp_timeseries <- ts(BRA_complete$Temperature_C, start = min(BRA_complete$Date), frequency = 12)
...
rain_decomp <- stl(rain_timeseries, s.window = "periodic")
plot(rain_decomp)  

temp_decomp <- stl(temp_timeseries, s.window = "periodic")
plot(temp_decomp)
...
rain_seasonal <- rain_decomp$time.series[,1]
rain_trend <- rain_decomp$time.series[,2]
rain_remainder <- rain_decomp$time.series[,3]
